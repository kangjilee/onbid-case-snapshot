# corex/onbid_api.py — 프록시 릴레이 + HTTP 직접 호출 지원
...
PROXY_URL = os.getenv("ONBID_PROXY_URL","").strip()  # 프록시 릴레이 URL
...
# 직접 호출용 시도 목록: FORCE_HTTP=1이면 onbid HTTP만
BASES = []
if KEY_ONBID:
...
PROXY_TIMEOUT = httpx.Timeout(connect=3.0, read=8.0, write=8.0, pool=3.0)
...
def _proxy_client():
    return httpx.Client(headers=HEADERS, timeout=PROXY_TIMEOUT, 
                       http2=False, follow_redirects=True, trust_env=False)
...
def fetch_onbid(raw_input: str):
    ...
    if PROXY_URL:
        proxy_plans = []
        if ids["mnmt"]: proxy_plans.append({"mnmt": ids["mnmt"]})
        if ids["plnm"] and ids["cltr"]: proxy_plans.append({"plnm_no": ids["plnm"], "cltr_no": ids["cltr"]})
        if ids["plnm"]: proxy_plans.append({"plnm_no": ids["plnm"]})
        
        cli = _proxy_client()
        try:
            for plan in proxy_plans:
                try:
                    r = cli.get(PROXY_URL, params=plan)
                    tried_urls.append(str(httpx.Request("GET", PROXY_URL, params=plan).url))
                    logging.info(f"[PROXY] {r.status_code} {PROXY_URL} {_mask(plan)}")
                    r.raise_for_status()
                    ...
                    if ("service key is not registered" in low) or ("<resultcode>30</resultcode>" in low):
                        last_err = "KEY_NOT_REGISTERED(proxy)"; continue
                    ...
                    if not items: 
                        last_err = "NO_ITEMS(proxy)"; continue
                    
                    item = items[0] if isinstance(items,list) else items
                    norm = normalize_unify_item(item)
                    
                    return norm, {"ok": True, "via": "proxy", "base": PROXY_URL, "tried_urls": tried_urls}
                    
                except Exception as e:
                    last_err = f"PROXY_ERROR:{type(e).__name__}:{e}"; time.sleep(0.25); continue
        finally:
            cli.close()
    
    # 2) 직접 호출 모드 (백업 - 현재 회사망에서는 막힘)
    plans = []
    if ids["mnmt"]: plans.append({"CLTR_MNMT_NO": ids["mnmt"]})
    ...
    for p in plans:
        for base,key,fam in BASES:
            ...
            try:
                ...
                return norm, {"ok":True, "via": "direct", "base":base, "tried_urls":tried_urls}
            except Exception as e:
    ...
    # 프록시 상태 표시
    via_info = "proxy+direct" if PROXY_URL else "direct_only"
    return None, {"ok":False, "error":last_err or "ALL_FAILED", "via":via_info, "tried_urls":tried_urls}